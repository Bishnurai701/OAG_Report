...
@RestController
@RequestMapping("/api/account/hris/report")
@PreAuthorize("hasRole('ADMIN') or hasRole('OAG') or hasRole('DTCO') or hasRole('PTCO') or hasRole('OAG_USER')")
public class HrisReportController {
	...
	public ResponseEntity<?> getDeductionReport(@Valid @RequestBody ReportRequestModel request) {
		return ResponseEntity.ok().body(reportService.getDeductionReport(request));
	}
	...
	public ResponseEntity<?> getPaymentDeductionReport(@Valid @RequestBody ReportRequestModel request) {
		return ResponseEntity.ok().body(reportService.getPaymentDeductionReport(request));
	}
